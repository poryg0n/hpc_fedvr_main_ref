...
!                 nmax, nmax_, krange, ios, nt, order, src_type
      ...
      complex(8), allocatable, dimension(:) :: psi_exact,             &
                                               psi0, psi, psi_x,      &
                                               psi_in, psi_out,       &
                                               phi_in, phi_out,       &
                                               phi_inc,       &
...
!     phi_inx = phi_in
      ...
      norm_ref2(1) = sqrt(sum(abs(wfc0(:,1) * wx * dsqrt(jacc))**2))
      norm_ref2(2) = sqrt(sum(abs(wfc0(:,2) * wx * dsqrt(jacc))**2))

      norm_ref1(1) = sqrt(sum(abs(wf0(:,1))**2))
      norm_ref1(2) = sqrt(sum(abs(wf0(:,2))**2))
      ...
      do i=1,nmax_
         write(init_unit,*) i, xx(i),                                  &
                         real(wfc0(i,1)), imag(wfc0(i,2)), omega(2)
      enddo
...
!     pause
      ...
      split_time = 0.d0
       ...
       write(*,'(a,i0)') "Maximum threads    : ", omp_get_max_threads()
...
!        write(*,*) "who treats this part?", tid
         ...
         norm_1 = sqrt(sum(abs(wf(:,1))**2))
         norm_2 = sqrt(sum(abs(wf(:,2))**2))
...
!        norm_ref=norm_2-1.d0
         ...
         if (do_time_obs) then
            ...
            if (mod(i, obs_stride) == 0) then
               ...
               write(log_unit,'(2i8,*(1x,7e20.10))') kobs, nobs, tt,    &
                                        norm_(nch__),                &
                                        p0, pexc, pion,                &
                                        nrg_, xt_, pt_
